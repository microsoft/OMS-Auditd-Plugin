...
class LinuxRPMFile:
    ...
    def GenerateSpecFile(self):
        ...
        for d in self.sections["Directories"]:
            if d.type != "sysdir":
                specfile.write('%defattr(' + str(d.permissions) + "," + d.owner + "," + d.group + "," + str(d.permissions) + ")\n")
    ...
    def StageAndProperlyNameRPM(self):
        if 'OUTPUTFILE' in self.variables:
            ...
            rpmNewFileName = self.variables['SHORT_NAME'] + '-' + \
                self.fullversion_dashed + '.' + self.variables['PFARCH'] + '.rpm'
    ...
    def BuildPackage(self):
        ...
        targetArch = ""
        if self.variables["PFARCH"] == "x86_64":
            targetArch = "--define \"_target_processor x86_64\" --target x86_64"
        elif self.variables["PFARCH"] == "aarch64":
            targetArch = "--define \"_target_processor aarch64\" --target aarch64"

        retval = os.system("rpmbuild " + targetArch + " --buildroot " + self.stagingDir + " -bb " + self.specfileName)
        if retval != 0:
