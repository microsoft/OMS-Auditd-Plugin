...
static struct bpf_program *bpf_sys_enter_open = NULL;
static struct bpf_program *bpf_sys_exit_open  = NULL;

static struct bpf_link    *bpf_sys_enter_open_link = NULL;
static struct bpf_link    *bpf_sys_exit_open_link  = NULL;
...
static void bpf_close_all(){
    ...
    bpf_link__destroy(bpf_sys_enter_open_link);
    bpf_link__destroy(bpf_sys_exit_open_link);
...
static void print_bpf_output(void *ctx, int cpu, void *data, __u32 size)
{
    ...
    {   
        printf("PID:%u SYS:%llu RET:%lld path=%s", event->pid, event->syscall_id, event->return_code, event->path);
    } else {
        bad_events++;
        printf("bad data arrived: expected size=%ld, actual size=%d", sizeof(event_s), size);
...
int main(int ac, char *argv[])
{
    ...
    if ( 
          ( NULL != ( bpf_sys_enter_open = bpf_object__find_program_by_title(bpf_obj,"tracepoint/syscalls/sys_enter_open")))  &&
          ( NULL != ( bpf_sys_exit_open  = bpf_object__find_program_by_title(bpf_obj,"tracepoint/syscalls/sys_exit_open")))   )
    {
        bpf_program__set_type(bpf_sys_enter_open, BPF_PROG_TYPE_TRACEPOINT);
        bpf_program__set_type(bpf_sys_exit_open, BPF_PROG_TYPE_TRACEPOINT);
    } else {
    ...
    bpf_sys_enter_open_link = bpf_program__attach_tracepoint(bpf_sys_enter_open, "syscalls", "sys_enter_open");
    bpf_sys_exit_open_link = bpf_program__attach_tracepoint(bpf_sys_exit_open, "syscalls", "sys_exit_open");
        
    if ( (libbpf_get_error(bpf_sys_enter_open_link)) || 
         (libbpf_get_error(bpf_sys_exit_open_link))  )
        return 2;
